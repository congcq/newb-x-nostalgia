...
vec3 renderOverworldSky(nl_skycolor skycol, vec3 viewDir) {
  ...
  if(viewDir.y >= 0.0){
  viewDir.y = pow(abs(viewDir.y), 0.55);
  }
  if(viewDir.y < 0.0){
  viewDir.y = -pow(abs(1.5*viewDir.y), 1.0);
  }
  ...
  float gradient2 = 0.18*gradient1 + 0.08*hsq;
  gradient1 *= gradient1;
  vec3 sky = mix(skycol.horizon, skycol.horizonEdge, gradient1);
  sky = mix(skycol.zenith, skycol.horizon, gradient2);
  if(viewDir.y < 0.0){
  sky *= mix(mix(mix(mix(1.0, 0.8,smoothstep(0.0, -0.25, viewDir.y)), 0.6, smoothstep(0.0, -0.5, viewDir.y)),0.4,smoothstep(0.0, -0.75, viewDir.y)), 0.25, smoothstep (0.0, -1.0, viewDir.y));
  }
  return sky;
}
...
vec3 getSunBloom(float viewDirX, vec3 horizonEdgeCol, vec3 FOG_COLOR) {
  ...
  float spread = smoothstep(0.0, 2.5, abs(viewDirX*viewDirX*viewDirX*viewDirX*viewDirX*viewDirX));
...
float sunBloomS = smoothstep(0.9,1.0,sunDir.z*0.915*scale);
float spread = smoothstep(0.0, 1.5, (sunDir.z));


sunBloomS = spread*spread;
sunBloomS = min(length(spread), 1.0);
sunBloomS += max(length(spread)*0.5, 0.1);
vec3 sunCol = NL_END_HORIZON_COL*10000.0*sunShape;
sunCol += sunCol;
sunCol *= smoothstep(0.0, 0.1, sunShape);
vec3 sunBloom = 3.0*NL_END_HORIZON_COL*1.0*sunBloomS;
...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  ...
  vec3 sky= horizonCol+zenithCol;
  sky *= mix(zenithCol,horizonCol,0.1);
sky += renderSun(viewDir.zyx,445.0, 1.0, 0.0, 0.25)*0.5;
  return sky;
}
...
//viewDir.y = -viewDir.y;
  ...
  if (!(env.underwater || env.nether)) {
    ...
    specular = clamp(length(specular), 0.0, 3.0);
